...
if ($escolha1 == 1) {
    do {
        ...
        if (is_numeric($escolhaOrdenador) == false || $escolhaOrdenador > 4 || $escolhaOrdenador < 1) {
            echo VERMELHO . "A opção escolhida não é valida"  . "\n";
            $sair = false;
        } else {
            $sair = true;
    ...
    switch ($escolhaOrdenador) {
        ...
        case 3:
            $inicio = microtime(true);
            $listaOrdenada = $lista;
            merge_sort($listaOrdenada, 0, (count($listaOrdenada) - 1));
            $fim = microtime(true);
            $tempo = $fim - $inicio;
            $tempo = number_format($tempo, 5, '.', ',');
            echo "O algoritmo demorou $tempo segundos para ordenar e os dados estão guardados no arquivo merge.csv";
            guardarListaOrdenada($listaOrdenada, 3);
            break;
        case 4:
            $inicio = microtime(true);
            $listaOrdenada = $lista;
            quicksort($listaOrdenada, 0, (count($listaOrdenada) - 1));
            $fim = microtime(true);
            $tempo = $fim - $inicio;
            $tempo = number_format($tempo, 5, '.', ',');
            echo "O algoritmo demorou $tempo segundos para ordenar e os dados estão guardados no arquivo quicksort.csv";
            guardarListaOrdenada($listaOrdenada, 4);
            break;
    }
} else {
    do {
